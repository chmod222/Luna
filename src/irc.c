...
/*
int
irc_privmsg(int fd, const char *receiver, const char *fmt, ...)
{
    va_list args;
    ...
    return net_sendfln(fd, "PRIVMSG %s :%s", receiver, line);
}
...
int irc_notice(int fd, const char *receiver, const char *fmt, ...)
{
    ...
    return net_sendfln(fd, "NOTICE %s :%s", receiver, line);
}
...
int irc_respond(int fd, struct irc_event *ev, const char *fmt, ...)
{
    ...
    {
        return irc_notice(fd, ev->from.nick, "%s: %s",
                ev->from.nick,
                line);
    }
